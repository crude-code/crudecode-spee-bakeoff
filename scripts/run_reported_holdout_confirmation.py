#!/usr/bin/env python3
"""Fixed-profile confirmation on a provenance-gated reported-holdout board.

The profile is fixed to ``gated_cohort_v1`` and the role is fixed to ``confirm``.
The board manifest must prove that all scored holdout months are classified
REPORTED.  Reporting-method and producing-day fields remain validation-only;
they are never provided to the forecasting model.
"""
...
def main() -> int:
    ...
    ap.add_argument("--board", default="board/private-reported-holdout")
    ...
    ap.add_argument("--out", default="board/results/reported-holdout-confirm")
    ...
    provenance_gate = manifest.get("provenance_gate", {})
    if provenance_gate.get("mode") not in {"reported_holdout", "reported_full"}:
        raise SystemExit(
            "board must be extracted with --provenance-mode reported_holdout or reported_full"
        )
    if float(provenance_gate.get("min_holdout_reported_fraction", 0.0)) < 1.0:
        raise SystemExit("board does not guarantee a fully REPORTED holdout")
    ...
    original = load_module(legacy_src / "forecast_benchmark/arps.py", "original_scipy_reported_holdout")
    ...
    report = {
        "schema_version": 1,
        "protocol": "one fixed gated_cohort_v1 profile on one provenance-gated REPORTED holdout confirmation",
    ...
    (out / "reported_holdout_confirmation.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
