#!/usr/bin/env python3
"""One-look confirmation of the pre-registered ``gated_cohort_v1`` profile.
...
def main() -> int:
    ...
    ap.add_argument("--board", default="board/private-v14")
    ...
    ap.add_argument("--out", default="board/results/v14-confirm3")
    ...
    original = load_module(legacy_src / "forecast_benchmark/arps.py", "original_scipy_v14_confirm3")
    ...
    report = {
        "schema_version": 1,
        "protocol": "one fixed v1.4 profile, one untouched confirm3 look, one-sided decision with tail guardrails",
    ...
    (out / "v14_confirmation.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
